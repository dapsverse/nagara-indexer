...
/**
 * `8n` / `0xffn` in the indexer's copy. This app's tsconfig targets ES2017, where
 * BigInt literals do not compile, so the same constants are built with BigInt().
 * The arithmetic is identical — this is the one intentional divergence.
 */
...
export function decodeStandardTransfer(
  ...
  const readU128 = (offset: number) => {
    ...
    for (let i = 15; i >= 0; i -= 1) {
      value = (value << BYTE_SHIFT) | BigInt(body[offset + i]);
    }
...
export function encodeStandardTransfer(
  ...
  for (let i = 0; i < 16; i += 1) {
    amount[i] = Number(value & BYTE_MASK);
    value >>= BYTE_SHIFT;
  }
...
export const rawSelector = (name: string) =>
  blake2AsU8a(name, 256).subarray(0, 4);
